...
> **Phase 1 (In-Scope):** 15 epics for core identity resolution system with blocking key matching, tier assignment, and 9 payment adapters.
>
> **Phase 2:** See [stories_phase2.md](stories_phase2.md) — deferred stories for manual review queue, API rate limiting, and metrics dashboard.
>
> **Phase 3 (Deferred):** See [epics_phase3.md](epics_phase3.md) and [stories_phase3.md](stories_phase3.md) — household formation, probabilistic ML, payment network analysis, compliance reporting.
>
> **Phase 4 (Exploratory):** See [epics_phase4.md](epics_phase4.md) and [stories_phase4.md](stories_phase4.md) — graph-based resolution, cross-border reconciliation, SMPC/ZKP privacy, biometric signals, quantum-ready cryptography.
>
...
Implement the stateless pipeline that orchestrates candidate generation from the Redis blocking index, bucket/tier assignment (Tier 1–55) based on which blocking keys matched, automated decisions (auto-merge, auto-stub, or manual review routing), and audit logging to the match_decision_log. *Note: Manual review queue (S-04-05) deferred to Phase 2.*
...
Build the external-facing REST and GraphQL API for FI developers to query resolved entity data. Strictly scoped to a single FI per OAuth token. Exposes entity profiles, linked accounts, household groupings, and resolution metadata with SLO of P99 < 200ms. *Note: Rate limiting and OAuth enforcement (S-06-04) deferred to Phase 2.*
...
Production-grade observability: structured OpenTelemetry logging, distributed tracing across the full resolution pipeline, immutable entity audit log, Kubernetes Helm chart, and a 10k events/minute load test harness. *Note: Metrics dashboard (S-08-02) deferred to Phase 2.*
